...
#The build 
	...
	make -C $(pwd) O=output msm8916_sec_defconfig VARIANT_DEFCONFIG=j5fgm_defconfig SELINUX_DEFCONFIG=selinux_defconfig
...
#Ramdisk packing
	...
	cp -r hK-tools/ramdisk/J5M/* hK-out/pack/rd
...
sed -i 's/A500xx/J500M/g' $(pwd)/hK-out/zip/META-INF/com/google/android/aroma-config
...
cd hK-out/zip
zip -r -9 - * > ../../hK-zip/"J500M$(cat ../../.scmversion).zip"
cd ../../
